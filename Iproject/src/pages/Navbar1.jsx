...
import { BiHome, BiSolidBadge, BiSolidBadgeCheck, BiSolidHeart } from "react-icons/bi";
...
export default function Post() {
  ...
  useEffect(() => {
    ...
    console.log(user.image, 'ini gambar');
  return (
    <>
  ...
  .map((el, i) => (
          <div className="bg-white border rounded-sm max-w-md my-2" key={i}>
            <div className="flex items-center px-4 py-3">
              ...
              />
              <div className="ml-3">
                {el.User.status === "Premium" && (
                  <span className="text-base font-semibold antialiased flex" key={el.User.id} >
                    {el.User.fullName} <BiSolidBadgeCheck className='ms-1 mt-1' style={{color: "#22d3ee", fontSize: "20px"}}/>
                  </span>
                )}
                {el.User.status === "Free" && (
                  <span className="text-base font-semibold antialiased block leading-tight" key={el.User.id}>
                    {el.User.fullName} 
                  </span>
                )}
              </div>
            ...
            <p className="pl-6">
              ...
              {el.description.length > 100 ? (isExpanded ? el.description : `${el.description.slice(0, 100)}...`) : el.description}
              {el.description.length > 100 && (
                <button onClick={toggleExpand} style={{opacity: "70%"}}>
                  {isExpanded ? " ...secukupnya" : "selengkapnya"}
                </button>
              )}
            </p>
          </div>
